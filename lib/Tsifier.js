...
module.exports = function (ts) {
	...
	function parseOptions(opts, bopts) {
		...
		if (configFile) {
			...
			files = configFileContents.files || [];
		} else {
			files = [];
		}
		...
		if (configFile) {
			// Resolve `files` relative to tsconfig.json
			parsed.fileNames = parsed.fileNames.map(function (filename) {
				var result = path.dirname(configFile) + '/' + filename;
				return result;
			});
		}
	...
	Tsifier.prototype.generateCache = function (files) {
		...
		// :PERFORMANCE: Cache ts program between file changes.
		if (!this.program) {
			this.createProgram();
		} else {
			// :TODO: Check if entrypoint files changed since the initial entrypoint set.
		}
		this.compile();
	};
	...
	Tsifier.prototype.createProgram = function () {
		var self = this;
		var rootFilenames = [];
		...
		self.rootFilenames = rootFilenames;
		log('Compiling files:');
		rootFilenames.forEach(function (file) { log('  %s', file); });

		var createProgram_t0 = time.start();
		self.program = ts.createProgram(self.rootFilenames, self.opts, self.host);
